...
@task_bp('/tasks', methods=['GET'])
@jwt_required
def get_tasks():
    user_id = get_jwt_identity()
    tasks = Task.query.filet_by(user_id=user_id).all()
    result = [{'id':t.id, 'title':t.title, 'description':t.description, 'completed':t.completed} for t in tasks]
    return jsonify(result)

@task_bp('/tasks/<int:task_id>', methods=['PUT'])
@jwt_required
def update_tasks(task_id):
    data = request.get_json()
    user_id = get_jwt_identity()
    task = Task.query.filter_by(id=task_id, user_id=user_id).all()
    if not task:
...
@task_bp('/tasks/<int:task_id>', methods=['DELETE'])
@jwt_required
